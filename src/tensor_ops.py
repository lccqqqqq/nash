...
def mpo_to_full(M):
    ...
    M_full = M[0].squeeze(axis=0)
    for i in range(0,L-1):
        M_full = np.tensordot(M_full,M[i+1],axes=(2*i,0))
   
    M_full=M_full.squeeze()
    nd = M_full.ndim
    #M_full = M_full.transpose(list(range(0, nd, 2)) + list(range(1, nd, 2)))
    M_full = np.reshape(M_full,[d**(L),d**(L)])
...
def svd_theta(theta, truncation_par,return_XYZ=None,normalize=True):
    ...
    U, s, V = sp.linalg.svd( theta, compute_uv=True, full_matrices=False, lapack_driver='gesvd')
    s[np.abs(s)<10e-14] = 0.
    nrm = np.linalg.norm(s)
    eta = np.min([ np.count_nonzero((1 - np.cumsum(s**2)/nrm**2) > truncation_par['p_trunc'])+1, truncation_par['chi_max']])
    nrm_t = np.linalg.norm(s[:eta])
...
def compress(Psi,chi, normalize=True):
    ...
    def sweep(Psi, truncation_par, normalize=True):
        ...
        for j in range(L-1):
            ...
            theta = np.tensordot( np.eye(d**2).reshape(d,d,d,d), theta, axes = [[2, 3], [0, 2] ]) #Theta = U Theta
    ...
    chi_max = np.max([A.shape for A in Psi])
       
    p_trunc, nrm  = sweep(Psi,{'p_trunc':10e-14, 'chi_max': chi_max},normalize=normalize)
   
    Psi = mps_invert(Psi)
    p_trunc, nrm = sweep(Psi,{'p_trunc':10e-14, 'chi_max': chi},normalize=normalize)
    Psi = mps_invert(Psi)
...
def get_transfer_MPO(M):
    ...
    for t in M:
        _,a,b,c,d = t.shape
        T.append(np.tensordot(t,np.conj(t),axes=(0,0)))
        T[-1] = T[-1].transpose(0,4,1,5,2,6,3,7)
...
def mpo_to_full(M):
    ...
    M_full = M[0].squeeze(axis=0) # v1 h1 h2
    for i in range(0,L-1):
        M_full = np.tensordot(M_full,M[i+1],axes=(2*i,0)) # h1 h2 v1 h3 h4 ...
